...
export default class GetOauthAccount {
    static async byProvider_providerUserId(client: Prisma.TransactionClient, provider: OauthAccount['provider'], providerUserId: OauthAccount['providerUserId']) {
    ...
    static async byUserUuid_provider(client: Prisma.TransactionClient, user: User, provider: OauthAccount['provider']) {
        return await client.oauthAccount.findUniqueOrThrow({
